...
- **2026-08-18 M5 擦拭耦合剂（复用 M2 UGUI 骨架，单步交互结束模块）**：
  ...
  - **2026-08-23 工具架用 MainScene/Tool（老板定稿，方案 B）**：老板手工添加 MainScene/Tool（M2 样式三槽位 ProbeHome/RulerHome/RagHome，含 bg/Chip/Outline）。M5Setup：`Tool` 存在时跳过 ToolShelf 创建；`EnsureToolCompat` 适配 Tool 树——Probe/Ruler 仅静态展示（M2ProbeDrag/M2RulerDrag 由 Adapt 移除，M5 不参与交互），RagHome 槽位工具节点（复制残留名为 Ruler）改名为 Rag + rag.png/置灰/Outline/M5RagDrag（擦拭功能），Rag 尺寸改方形 80×80（150×32 扁条会挤压方形 rag.png 成重影感）；EnsureAll 的 rag/ragHome 查找优先 Tool 树，无 Tool 回退标准 ToolShelf。Tool 树布局 Scene 权威不覆盖。
...
- **Unity 6 伪 null 对象与 `??` 不兼容（2026-08-18 M5Setup 实战）**：`GetComponent<T>()`/`Find` 对缺失对象返回 **Unity 伪 null**（非 C# null，`== null` 为 true 但引用非空），`??` 运算符检查 C# 引用不触发，导致 `var cg = go.GetComponent<CanvasGroup>() ?? go.AddComponent<CanvasGroup>();` 得到伪 null 组件，访问属性抛 `MissingComponentException`。**必须用 `if (x == null)` 分步**：`var cg = go.GetComponent<CanvasGroup>(); if (cg == null) cg = go.AddComponent<CanvasGroup>();`。同理 `GetComponent<T>() ?? AddComponent<T>()` 全部禁止。
  - **2026-08-23 Bootstrap 复发（M3DigitalHumanBootstrap BuildViews/装配）**：`vp = GetComponent<VideoPlayer>() ?? AddComponent<VideoPlayer>()` 及 M1DeepSeekClient/M1QAPanel/M1DigitalHumanPresenter 三处同坑，Play 时数字人装配抛 `MissingComponentException` 中断（try-catch 已捕获日志）——全部改 `if == null` 分步，M3/M4 亦受益。
- **`TextAlignmentOptions` 无 `MiddleCenter`**：TMP 枚举为 `Center`/`MidlineLeft`/`MidlineRight` 等；`MiddleCenter` 编译报 CS0117（2026-08-18 M5Setup）。
- **UGUI `Outline` 继承 `Shadow`（2026-08-23 M2/M3 探头）**：`GetComponent<Shadow>()` 可能返回 `Outline`，造成真正阴影未配置或参数写到描边。探头必须精确筛选 `GetComponents<Shadow>()` 中 `GetType() == typeof(Shadow)`，再独立获取/创建 `Outline`；三模块共用 `M2ProbeDrag.ApplyProbeEffects(Image)`。工具架去除卡片框只能处理 `ProbeHome/RulerHome` 直接 `bg` 的 `Image` 与其 Outline，禁止遍历禁用整个 ToolShelf 子树，否则会误关探头/尺子自身描边。
