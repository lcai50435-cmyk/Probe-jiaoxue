...
2. **M1 台词**（Slide 1-2）：AI回答 显示的 6 条台词按 PPT 更新（初始/选错/选对 × M1-1/M1-2），含 M1Setup 规范化与 M1ToolSelection 默认值；M1 初始台词为“那我们开始选择探测仪器吧，有问题随时长按我进行提问哦！”。
3. **M1 引导期间暂隐对话框**（老板 2026-08-24 增量）：引导视频播放期间，常驻数字人和白板数字人对话框一并隐藏；视频正常播放结束或跳过时恢复。复用 `M1IntroVideo` 的隐藏/恢复链路，旧场景直接 Play 生效，且不修改 Scene YAML。
4. **M2/M3/M4 底部提示文案**（DefaultHints）：按 PPT 各模块「改成」条目替换；M2 的涂抹耦合剂提示删除（改由气泡承载）。
...
10. **M5 数字人云朵台词**（老板 2026-08-23 增量）：复用 `ModuleSpeechBubble` 与现有 `DigitalHumanStage/dialog` 云朵，开始显示“根据《安规》规定“\n焊缝探伤作业后钢轨顶面上的焊缝探伤耦合剂必须擦除干净。””（首个“焊缝探伤”从第二行开始），擦拭完成显示“恭喜你！完整掌握了“三位一体、交叉验证”新工艺！”，重置恢复开始台词；M5 专属文字区 `anchorOffset=(-384,1)`，其余位置、尺寸和非缩放计时与 M2/M3/M4 一致，M5 Scene 不写回。
11. **M4→M5 完成链路**（老板 2026-08-24 增量）：M4 完成后点击“下一模块”直接加载 M5；`M4FlowController.nextSceneName` 默认 `M5`，M5 必须列入 Build Settings；运行时烟测须点击真实按钮并断言 M5 主流程已启动。
...
- [x] M1 引导播放期间白板数字人对话框随常驻数字人隐藏，播完或跳过后恢复
...
- [x] M5 开始/完成/重置按指定时机显示《安规》与工艺掌握台词，复用 M2/M3/M4 云朵位置和尺寸，且不写入 M5 Scene
- [x] M4 点击“下一模块”后进入 M5，且 M5 主流程已启动
