...
- **2026-08-23 M5 工具架用 MainScene/Tool（方案 B）**：老板手工添加 Tool（M2 样式三槽位 ProbeHome/RulerHome/RagHome），M5Setup 在 Tool 存在时跳过 ToolShelf 创建；Probe/Ruler 仅静态展示（M2ProbeDrag/M2RulerDrag 移除），RagHome 槽位工具节点改名为 Rag + rag.png/M5RagDrag（擦拭功能）；EnsureAll 查找优先 Tool 树。详见 low-code.md 5.4。
