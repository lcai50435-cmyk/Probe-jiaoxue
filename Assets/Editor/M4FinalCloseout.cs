...
{
    /// <summary>M4 一次性收口：补角度 Slider、Probe/Ruler 的 bg 子节点、完成面板与当前距离文本，
    /// 挂载 M4 runtime 组件并配置引用，保存后记录前后 SHA-256。只操作 M4 Scene，不碰 M1/M2/M3。</summary>
    public static class M4FinalCloseout
    {
        ...
        [MenuItem("Tools/M4/Final Closeout")]
        public static void Run()
        {
            ...
            if (canvas == null) { Debug.LogError("[M4FinalCloseout] 未找到 Canvas，中止。"); return; }
            ...
            Debug.Log($"[M4FinalCloseout] 完成。before={before}");
            Debug.Log($"[M4FinalCloseout] 完成。after ={after}");
        }

        private static void EnsureAngleSlider(GameObject root)
        {
            var track = FindDeep(root, "AngleTrack");
            if (track == null) { Debug.LogError("[M4FinalCloseout] 未找到 AngleTrack。"); return; }
        ...
        private static void EnsureProbeBg(GameObject root)
        {
            ...
            if (probe == null) { Debug.LogError("[M4FinalCloseout] 未找到 Probe。"); return; }
            EnsureBgChild(probe);
        }
        ...
        private static void EnsureRulerBgAndHome(GameObject root)
        {
            ...
            if (ruler == null || home == null) { Debug.LogError("[M4FinalCloseout] 未找到 Ruler/RulerHome。"); return; }
            EnsureBgChild(ruler);
        ...
        private static GameObject EnsureCompletionPanel(GameObject root)
        {
            ...
            if (dock == null) { Debug.LogError("[M4FinalCloseout] 未找到 ControlDock_D。"); return null; }
        ...
        private static TMP_Text EnsureCurrentDistanceText(GameObject root)
        {
            ...
            if (header == null) { Debug.LogError("[M4FinalCloseout] 未找到 WaveHeader。"); return null; }
        ...
        private static void MountAndWire(GameObject canvas, GameObject completionPanel, TMP_Text currentDistanceText, GameObject helpPanel)
        {
            ...
            var flow = EnsureComponent<M4FlowController>(mount);
            var idle = EnsureComponent<M4IdleHelp>(mount);
            ...
            var probeDrag = probe != null ? EnsureComponent<M4ProbeDrag>(probe) : null;
            var rulerDrag = ruler != null ? EnsureComponent<M4RulerDrag>(ruler) : null;
            ...
            if (waveform != null)
            {
                ...
                waveform.appearMm = 55f; waveform.peakMm = 45f; waveform.stopMm = 40f;
            }
        ...
        private static string Sha256(string path)
        {
            ...
            catch (Exception e) { Debug.LogError("[M4FinalCloseout] 读取失败：" + e.Message); return "<unavailable>"; }
        }
    }
}
