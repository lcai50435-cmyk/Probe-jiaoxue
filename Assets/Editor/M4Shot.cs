...
{
    /// <summary>以三种审核视口渲染已保存的 M4 静态场景。</summary>
    public static class M4Shot
    {
        ...
        [MenuItem("Tools/M4/Capture Review Shots %#&9")]
        public static void CaptureAll()
        {
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(ScenePath) == null)
                throw new FileNotFoundException("请先运行 M4 Setup。", ScenePath);
            ...
            if (canvas == null) throw new InvalidOperationException("[M4Shot] 未找到 Canvas。");
            var waveFx = UnityEngine.Object.FindFirstObjectByType<M2WaveformFx>(FindObjectsInactive.Include);
            if (waveFx != null) waveFx.SetDistanceMm(55f);

            var savedMode = canvas.renderMode;
            var savedCamera = canvas.worldCamera;
            var cameraObject = new GameObject("M4ShotCamera", typeof(Camera));
            ...
            if (ComputeHash(ScenePath) != sceneHash)
                throw new InvalidOperationException("[M4Shot] 截图前后 M4 Scene 哈希发生变化。");
            Debug.Log("[M4Shot] 三视口非空截图完成，Scene 哈希不变。");
        ...
        private static void Capture(Camera camera, Canvas canvas, int width, int height, string path)
        {
            ...
            try
            {
                ...
                Debug.Log("[M4Shot] 已保存 " + width + "x" + height + " -> " + path);
            }
            finally
            {
        ...
        private static void ForceWaveMesh()
        {
            ...
            Debug.Log($"[M4Shot] wave vertices={helper.currentVertCount}");
        }
        ...
        private static void AssertNonBlank(Texture2D texture)
        {
            ...
            throw new InvalidOperationException("[M4Shot] 截图像素近似纯色，判定渲染失败。");
        }
