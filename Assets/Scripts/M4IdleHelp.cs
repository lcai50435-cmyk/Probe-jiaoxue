...
namespace M4
{
    /// <summary>防卡死帮助：只读 Flow 阶段并编排自动演示（55→40mm）。</summary>
    public class M4IdleHelp : MonoBehaviour
    {
        ...
        private IEnumerator DemoRoutine()
        {
            ...
            if (stage == M4FlowController.Stage.Positioning && probeDrag != null)
            {
            ...
            else if (stage == M4FlowController.Stage.Scanning && probeDrag != null)
            {
            ...
            if (flow != null)
            {
                ...
                if (flow.CurrentStage != M4FlowController.Stage.Positioning) probeDrag?.SetAngleLocked(true); // 扫描后角度锁定
            }
